...
const midiStore = new Map<string, MidiEntry>();
const backupStore = new Map<string, BackupEntry>();
const projects = new Map<string, string>([["default", process.cwd()]]);

const id = (prefix: string) =>
  `${prefix}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;

const resolveProjectPath = (projectId: string, relativePath: string) => {
  const base = projects.get(projectId);
  if (!base) {
...
const reportsDir = (projectId: string) => resolveProjectPath(projectId, "reports");

const safeUpdateHeader = (midi: Midi) => {
...
const markDirty = (entry: MidiEntry) => {
  entry.dirty = true;
...
const requireOne = (items: Array<[string, unknown]>) => {
...
const getEntry = (midiId: string) => {
  const entry = midiStore.get(midiId);
  if (!entry) {
...
const getTrack = (midi: Midi, trackId: number) => {
...
const inRange = (ticks: number, range?: { startTicks?: number; endTicks?: number }) => {
...
const getNoteChannel = (note: any, track: any) =>
  note.channel ?? track.channel ?? 0;

const getCcChannel = (cc: any, track: any) =>
  cc.channel ?? track.channel ?? 0;

const getPitchBendChannel = (pb: any, track: any) =>
...
const toEventList = (
  ...
  for (const [ccNumber, list] of Object.entries(track.controlChanges)) {
...
const gridToTicks = (grid: string | number, ppq: number) => {
...
const ticksToBbt = (midi: Midi, ticks: number) => {
  const ppq = midi.header.ppq;
  const timeSigs =
    midi.header.timeSignatures.length > 0
      ? [...midi.header.timeSignatures].sort((a, b) => a.ticks - b.ticks)
...
server.registerTool(
  ...
  withErrorHandling(async ({ backupId }) => {
    ...
    const midi = new Midi(backup.bytes);
    const midiId = id("midi");
    midiStore.set(midiId, {
      id: midiId,
...
server.registerTool(
  ...
  withErrorHandling(async ({ midiId }) => {
    ...
    const backup = backupStore.get(backupId);
    if (!backup) throw new Error(`backupId not found: ${backupId}`);
    entry.midi = new Midi(backup.bytes);
    safeUpdateHeader(entry.midi);
    entry.dirty = false;
    return ok("ok");
  })
...
server.registerTool(
  ...
  withErrorHandling(async ({ midiId }) => {
    const entry = getEntry(midiId);
    const tracks = entry.midi.tracks.map((t, index) => ({
      trackId: index,
      name: t.name,
      channel: t.channel,
...
server.registerTool(
  "replace_events",
  {
    ...
    description: "Replace events matching range/filter with provided events",
    inputSchema: z.object({
      midiId: z.string(),
      trackId: z.number().int().min(0),
      range: RangeSchema,
  ...
  withErrorHandling(async ({ midiId, trackId, range, filter, events }) => {
    const entry = getEntry(midiId);
    const track = getTrack(entry.midi, trackId);
    filterNotes(track, range, filter);
    filterCc(track, range, filter);
...
server.registerTool(
  ...
  {
    title: "Move events",
    description: "Move events matching range/filter by deltaTicks",
    inputSchema: z.object({
      midiId: z.string(),
      trackId: z.number().int().min(0),
      range: RangeSchema,
  ...
  withErrorHandling(async ({ midiId, trackId, range, filter, deltaTicks }) => {
    const entry = getEntry(midiId);
    const track = getTrack(entry.midi, trackId);
    for (const note of track.notes) {
      if (!inRange(note.ticks, range)) continue;
      if (!matchFilter("note", { midi: note.midi, channel: getNoteChannel(note, track) }, filter)) continue;
      note.ticks = Math.max(0, note.ticks + deltaTicks);
    }
    for (const [ccNumber, list] of Object.entries(track.controlChanges)) {
      const num = Number(ccNumber);
      for (const cc of list as any[]) {
        if (!inRange(cc.ticks, range)) continue;
        if (!matchFilter("cc", { number: num, channel: getCcChannel(cc, track) }, filter)) continue;
        cc.ticks = Math.max(0, cc.ticks + deltaTicks);
      }
    }
    for (const pb of track.pitchBends) {
      if (!inRange(pb.ticks, range) || !matchFilter("pitchbend", {channel: getPitchBendChannel(pb, track)}, filter)) {
        continue;
      }
      pb.ticks = Math.max(0, pb.ticks + deltaTicks);
    }
    markDirty(entry);
    return ok("ok");
  })
...
server.registerTool(
  ...
  {
    title: "Copy events",
    description: "Copy events from src to dst with deltaTicks",
    inputSchema: z.object({
      midiId: z.string(),
      ...
      range: RangeSchema,
      filter: FilterSchema,
      deltaTicks: z.number().int().default(0),
    }),
  },
  withErrorHandling(async ({ midiId, srcTrackId, dstTrackId, range, filter, deltaTicks }) => {
    const entry = getEntry(midiId);
...
const buildScale = (key: string, scale: string) => {
...
const constrainNote = (midi: number, allowed: Set<number>, strategy: string) => {
